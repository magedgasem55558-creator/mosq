...
halaqat.forEach(h => {
    halaqaFilter.innerHTML += `<option value="${h.id}">${h.name} - (الشيخ: ${h.teacherName})</option>`;
});
...
document.getElementById('saveRecitationBtn').addEventListener('click', async () => {
    ...
    try {
        await addDoc(collection(db, "records"), {
            ...
            fromAyah: fromAya || "0", toAyah: toAya || "0",
            grade,
            tomorrowRequirement: tomorrowReq || "لا يوجد",
            notes: notes || "",
            teacherPhone,
            pointsGiven: points,
